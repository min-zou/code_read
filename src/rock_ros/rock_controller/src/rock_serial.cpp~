...
bool rock_serial::open()
{
    return open(_portName, _baudrate, _flags);
}


bool rock_serial::bind(const char * portname, uint32_t baudrate, uint32_t flags)
{   
...
bool rock_serial::open(const char * portname, uint32_t baudrate, uint32_t flags)
{
    if (isOpened()) close();
    ...
    if (serial_fd == -1) 
    {
         fprintf(stderr,"Error, serial_fd = -1.\n");
         return false;
    }
    ...
    uint32_t termbaud = getTermBaudBitmap(baudrate);

    if (termbaud == (uint32_t)-1) {
        close();
        return false;
    }
...
int rock_serial::senddata(const unsigned char * data, size_t size)
{
...
int rock_serial::recvdata(unsigned char * data, size_t size)
{
    if (!isOpened()) return 0;
...
void rock_serial::flush( uint32_t flags)
{
    tcflush(serial_fd,TCIFLUSH); 
}

int rock_serial::waitforsent(uint32_t timeout, size_t * returned_size)
{
    if (returned_size) *returned_size = required_tx_cnt;
    return 0;
}

int rock_serial::waitforrecv(uint32_t timeout, size_t * returned_size)
{
...
int rock_serial::waitfordata(size_t data_count, uint32_t timeout, size_t * returned_size)
{
    size_t length = 0;
...
size_t rock_serial::rxqueue_count()
{
    if  ( !isOpened() ) return 0;
    size_t remaining;
...
uint32_t rock_serial::getTermBaudBitmap(uint32_t baud)
{
...
rock_rxtx * rock_rxtx::CreateRxTx()
{
    return new rock_serial();
}

void rock_rxtx::ReleaseRxTx(rock_rxtx *rxtx)
{
    delete rxtx;
}
