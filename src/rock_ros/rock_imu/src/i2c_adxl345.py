...
class i2c_adxl345:
	...
	def setTapThreshold(self, g=3):
		...
		# Range 0-8 g
		intervals = int(math.floor(g / 0.0625))
		if intervals < 256:
	...
	def setTapDuration(self, millisec=10):
		...
		intervals = int(math.floor(millisec * 1000/625))
		if intervals < 256:
	...
	def setTapLatency(self, millisec=150):
		...
		# Range 0-318
		intervals = int(math.floor(millisec / 1.25))
		if intervals < 256:
	...
	def setActivityThreshold(self, g=-1, axis='z', change=.5):
		...
		# Figure out g's and then intervals based on 62.5 mg
		# Range 0-16g
		intervals = int(math.floor(math.fabs(g) / 0.0625 ))
		print( intervals);
		if intervals < 256:
	...
	def setFreeFallTime(self, sec=.0500):
		...
		# Range 0-101
		intervals = int(math.floor(sec * 1000 / 5))
		if intervals < 256:
	...
	def addOption(self, register, *function_set):
		options = self.bus.readU8(register)
		for function in function_set:
			options = options | function
		self.bus.write8(register, options)
	...
	def getRawAxes(self):
		accel_x = self.bus.readS16(self.XAxisDataLSB)
		accel_y = self.bus.readS16(self.YAxisDataLSB)
		accel_z = self.bus.readS16(self.ZAxisDataLSB)
		return (accel_x, accel_y, accel_z)
	
	def getAxes(self):
		(accel_x, accel_y, accel_z) = self.getRawAxes()
		return (float(accel_x * 0.03125), float(accel_y * 0.03125), float(accel_z * 0.03125))
