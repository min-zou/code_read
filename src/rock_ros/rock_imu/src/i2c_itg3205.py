...
class i2c_itg3205:
	...
	def getDieTemperature(self):
		temp = self.bus.readS16BE(self.TempDataRegisterMSB)
	...
	def getAxes(self):
		gyro_x = self.bus.readS16BE(self.GyroXDataRegisterMSB) + self.gyro_x_bias
		gyro_y = self.bus.readS16BE(self.GyroYDataRegisterMSB) + self.gyro_y_bias
		gyro_z = self.bus.readS16BE(self.GyroZDataRegisterMSB) + self.gyro_z_bias
